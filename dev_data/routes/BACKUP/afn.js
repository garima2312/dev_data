...
var url = "mongodb://localhost:27017/mws_api_db25";
...
router.get('/',function(req,res,next){
				...
				mws.request(getReport, function(e, get_report) {
					...
					mws.request(report_info, function(e, report_info) {
						ne.each(report_info,function(index2, item){
							...
							db.collection("app_report").find(queryAppData).toArray(function(err5, result5){
							if (err5) throw err5;
								if (result5.length) { 
	...
	res.send('Product Inventory report has been saved !');
})
